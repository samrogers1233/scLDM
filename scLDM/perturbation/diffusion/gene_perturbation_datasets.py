"""Gene-perturbation dataset loader ."""
...
class MultimodalDataset_cell(Dataset):
    def __init__(
        ...
        if self.gene2vec_path is not None:
            self.gene_emb, _ = self._build_perturbation_embeddings(adata_pert)
        ...
        np.savez('/'.join(ae_path.split('/')[:-2]) + '/norm_factor19.npz',
                 ctrl_std=float(self.ctrl_std10), pert_std=float(self.pert_std10))
        print("done!")

    def _build_perturbation_embeddings(self, adata_pert):
        """Build per-cell gene2vec embeddings.

        Perturbation labels may be a single gene (e.g. "TP53") or a
        combination joined by "_" (e.g. "TP53_MDM2"). Tokens are averaged;
        tokens missing from gene2vec contribute zero to the sum.
        """
